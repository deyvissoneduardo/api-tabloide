# US-104 — Identificar cada QR Code
...
**Quero** identificar cada qr code  
...
Esta história pertence a **Supermercado**, domínio **QR Codes**, seção “B9. QR Code”. O requisito de origem é SUP-RF-092. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar identificar cada qr code no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita identificar cada qr code.

---

## 8. Fluxo principal

1. O ator acessa a consulta de identificar cada qr code.
...
- **RF-001:** O sistema deve identificar cada qr code.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "identificar cada qr code"
...
- `identificar_cada_qr_code_iniciado`: início intencional do fluxo.
- `identificar_cada_qr_code_concluido`: após confirmação do resultado.
- `identificar_cada_qr_code_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-092 — O supermercado deve poder identificar cada QR Code.`
