# US-103 — Gerar QR Codes diferentes por loja
...
**Quero** gerar qr codes diferentes por loja  
...
Esta história pertence a **Supermercado**, domínio **QR Codes**, seção “B9. QR Code”. O requisito de origem é SUP-RF-090. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar gerar qr codes diferentes por loja no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita gerar qr codes diferentes por loja.

---

## 8. Fluxo principal

1. O ator inicia gerar qr codes diferentes por loja.
...
- **RF-001:** O sistema deve gerar qr codes diferentes por loja.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "gerar qr codes diferentes por loja"
...
- `gerar_qr_codes_diferentes_loja_iniciado`: início intencional do fluxo.
- `gerar_qr_codes_diferentes_loja_concluido`: após confirmação do resultado.
- `gerar_qr_codes_diferentes_loja_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-090 — O supermercado deve poder gerar QR Codes diferentes por loja.`
