...
Esta história pertence a **Supermercado**, domínio **Produtos**, seção “B5. Produtos”. O requisito de origem é SUP-RF-038. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar informar uma descrição no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita informar uma descrição.

---

## 8. Fluxo principal

1. O ator inicia informar uma descrição.
...
- **RF-001:** O sistema deve informar uma descrição.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "informar uma descrição"
...
- `informar_descricao_iniciado`: início intencional do fluxo.
- `informar_descricao_concluido`: após confirmação do resultado.
- `informar_descricao_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-038 — O supermercado deve poder informar uma descrição.`
