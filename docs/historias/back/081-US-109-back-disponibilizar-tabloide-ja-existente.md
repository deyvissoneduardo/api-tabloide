# US-109 — Disponibilizar um tabloide já existente
...
**Quero** disponibilizar um tabloide já existente  
...
Esta história pertence a **Supermercado**, domínio **Tabloide existente**, seção “B19. Tabloide existente”. O requisito de origem é SUP-RF-160. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar disponibilizar um tabloide já existente no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita disponibilizar um tabloide já existente.

---

## 8. Fluxo principal

1. O ator inicia disponibilizar um tabloide já existente.
...
- **RF-001:** O sistema deve disponibilizar um tabloide já existente.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "disponibilizar um tabloide já existente"
...
- `disponibilizar_tabloide_ja_existente_iniciado`: início intencional do fluxo.
- `disponibilizar_tabloide_ja_existente_concluido`: após confirmação do resultado.
- `disponibilizar_tabloide_ja_existente_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-160 — O supermercado deve poder disponibilizar um tabloide já existente.`
