...
- **Dependências:** Back/US-056, Back/US-074
- **Bloqueia:** Nenhuma
- **Risco:** Baixo
...
Esta história pertence a **Supermercado**, domínio **Categorias**, seção “B4. Categorias”. O requisito de origem é SUP-RF-029. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar desativar categorias no escopo autorizado.
- Aplicar as regras de categorias documentadas nesta história.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita desativar categorias.

---

## 8. Fluxo principal

1. O ator seleciona o recurso e solicita desativar categorias.
...
**Aplicação:**  
Nos fluxos de categorias afetados por esta história.
...
- **RF-001:** O sistema deve desativar categorias.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "desativar categorias"
...
Rota: `/api/categorias/{id}/acoes/{acao}`
...
- nome: obrigatório; duplicidade permitida.
- descrição: opcional.
...
- `desativar_categorias_iniciado`: início intencional do fluxo.
- `desativar_categorias_concluido`: após confirmação do resultado.
- `desativar_categorias_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-029 — O supermercado deve poder desativar categorias.`
- Decisões aplicáveis: seção “Categorias” de `docs/historias/decisoes-mvp.md`.
- Histórias relacionadas: US-056, US-074.
