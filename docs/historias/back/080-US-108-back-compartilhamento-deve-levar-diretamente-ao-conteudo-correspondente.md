# US-108 — O compartilhamento deve levar diretamente ao conteúdo correspondente
...
**Quero** o compartilhamento deve levar diretamente ao conteúdo correspondente  
...
Esta história pertence a **Supermercado**, domínio **Compartilhamento**, seção “B15. Compartilhamento”. O requisito de origem é SUP-RF-141. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar o compartilhamento deve levar diretamente ao conteúdo correspondente no escopo autorizado.
...
2. O sistema aplica a qualidade “O compartilhamento deve levar diretamente ao conteúdo correspondente” antes de devolver o resultado.
...
- **RF-001:** O sistema deve o compartilhamento deve levar diretamente ao conteúdo correspondente.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "o compartilhamento deve levar diretamente ao conteúdo correspondente"
...
A capacidade é transversal e deve ser validada nos endpoints afetados de `/api/compartilhamentos`. Não cria endpoint exclusivo quando puder ser aplicada como requisito de qualidade.
...
- `compartilhamento_deve_levar_diretamente_ao_conteudo_correspondente_iniciado`: início intencional do fluxo.
- `compartilhamento_deve_levar_diretamente_ao_conteudo_correspondente_concluido`: após confirmação do resultado.
- `compartilhamento_deve_levar_diretamente_ao_conteudo_correspondente_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-141 — O compartilhamento deve levar diretamente ao conteúdo correspondente.`
