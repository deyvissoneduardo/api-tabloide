# US-046 — Possuir visão consolidada da operação
...
**Quero** possuir visão consolidada da operação  
...
Esta história pertence a **Administração da plataforma**, domínio **Analytics, dashboard e relatórios**. O requisito de origem é ADM-RNF-007. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar possuir visão consolidada da operação no escopo autorizado.
...
2. O sistema aplica a qualidade “O administrador deve possuir visão consolidada da operação” antes de devolver o resultado.
...
- **RF-001:** O sistema deve possuir visão consolidada da operação.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "possuir visão consolidada da operação"
...
- `possuir_visao_consolidada_operacao_iniciado`: início intencional do fluxo.
- `possuir_visao_consolidada_operacao_concluido`: após confirmação do resultado.
- `possuir_visao_consolidada_operacao_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RNF-007 — O administrador deve possuir visão consolidada da operação.`
