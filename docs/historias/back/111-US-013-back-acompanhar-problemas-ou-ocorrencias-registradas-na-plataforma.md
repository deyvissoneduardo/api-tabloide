# US-013 — Acompanhar problemas ou ocorrências registradas na plataforma
...
**Quero** acompanhar problemas ou ocorrências registradas na plataforma  
...
Esta história pertence a **Administração da plataforma**, domínio **Suporte e ocorrências**. O requisito de origem é ADM-RF-035. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar acompanhar problemas ou ocorrências registradas na plataforma no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita acompanhar problemas ou ocorrências registradas na plataforma.

---

## 8. Fluxo principal

1. O ator acessa a consulta de acompanhar problemas ou ocorrências registradas na plataforma.
...
- **RF-001:** O sistema deve acompanhar problemas ou ocorrências registradas na plataforma.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "acompanhar problemas ou ocorrências registradas na plataforma"
...
Rota: `/api/ocorrencias` ou `/api/ocorrencias/{id}`, conforme coleção ou detalhe.

Filtros e paginação são parâmetros de consulta. Sucesso: `200 OK`. Erros aplicáveis: 400, 401, 403, 404 e 500.
...
- `acompanhar_problemas_ou_ocorrencias_registradas_na_plataforma_iniciado`: início intencional do fluxo.
- `acompanhar_problemas_ou_ocorrencias_registradas_na_plataforma_concluido`: após confirmação do resultado.
- `acompanhar_problemas_ou_ocorrencias_registradas_na_plataforma_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-035 — O administrador deve poder acompanhar problemas ou ocorrências registradas na plataforma.`
