# US-069 — Identificar cada loja individualmente
...
**Quero** identificar cada loja individualmente  
...
Esta história pertence a **Supermercado**, domínio **Lojas**, seção “B3. Gestão de lojas”. O requisito de origem é SUP-RF-020. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar identificar cada loja individualmente no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita identificar cada loja individualmente.

---

## 8. Fluxo principal

1. O ator acessa a consulta de identificar cada loja individualmente.
...
- **RF-001:** O sistema deve identificar cada loja individualmente.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "identificar cada loja individualmente"
...
Rota: `/api/lojas` ou `/api/lojas/{id}`, conforme coleção ou detalhe.
...
- `identificar_cada_loja_individualmente_iniciado`: início intencional do fluxo.
- `identificar_cada_loja_individualmente_concluido`: após confirmação do resultado.
- `identificar_cada_loja_individualmente_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-020 — O supermercado deve poder identificar cada loja individualmente.`
