# US-173 — As métricas devem representar os eventos registrados corretamente
...
**Quero** as métricas devem representar os eventos registrados corretamente  
...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RNF-020. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar as métricas devem representar os eventos registrados corretamente no escopo autorizado.
...
2. O sistema aplica a qualidade “As métricas devem representar os eventos registrados corretamente” antes de devolver o resultado.
...
- **RF-001:** O sistema deve as métricas devem representar os eventos registrados corretamente.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "as métricas devem representar os eventos registrados corretamente"
...
A capacidade é transversal e deve ser validada nos endpoints afetados de `/api/metricas`. Não cria endpoint exclusivo quando puder ser aplicada como requisito de qualidade.
...
- `metricas_devem_representar_eventos_registrados_corretamente_iniciado`: início intencional do fluxo.
- `metricas_devem_representar_eventos_registrados_corretamente_concluido`: após confirmação do resultado.
- `metricas_devem_representar_eventos_registrados_corretamente_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RNF-020 — As métricas devem representar os eventos registrados corretamente.`
