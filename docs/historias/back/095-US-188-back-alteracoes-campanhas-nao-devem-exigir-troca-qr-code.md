# US-188 — Alterações de campanhas não devem exigir troca do QR Code
...
**Quero** alterações de campanhas não devem exigir troca do qr code  
...
Esta história pertence a **Supermercado**, domínio **QR Codes**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RNF-039. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar alterações de campanhas não devem exigir troca do qr code no escopo autorizado.
...
2. O sistema aplica a qualidade “Alterações de campanhas não devem exigir troca do QR Code” antes de devolver o resultado.
...
- **RF-001:** O sistema deve alterações de campanhas não devem exigir troca do qr code.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "alterações de campanhas não devem exigir troca do qr code"
...
- `alteracoes_campanhas_nao_devem_exigir_troca_qr_code_iniciado`: início intencional do fluxo.
- `alteracoes_campanhas_nao_devem_exigir_troca_qr_code_concluido`: após confirmação do resultado.
- `alteracoes_campanhas_nao_devem_exigir_troca_qr_code_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RNF-039 — Alterações de campanhas não devem exigir troca do QR Code.`
