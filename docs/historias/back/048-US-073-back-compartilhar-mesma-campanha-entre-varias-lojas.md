# US-073 — Compartilhar uma mesma campanha entre várias lojas
...
Estimativa relativa baseada no escopo descrito, nos cenários de validação e teste, no risco alto e em 2 dependência(s) identificada(s).
...
**Quero** compartilhar uma mesma campanha entre várias lojas  
...
Esta história pertence a **Supermercado**, domínio **Lojas**, seção “B3. Gestão de lojas”. O requisito de origem é SUP-RF-025. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar compartilhar uma mesma campanha entre várias lojas no escopo autorizado.
...
2. O sistema aplica a qualidade “O supermercado deve poder compartilhar uma mesma campanha entre várias lojas” antes de devolver o resultado.
...
- **RF-001:** O sistema deve compartilhar uma mesma campanha entre várias lojas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "compartilhar uma mesma campanha entre várias lojas"
...
A capacidade é transversal e deve ser validada nos endpoints afetados de `/api/lojas`. Não cria endpoint exclusivo quando puder ser aplicada como requisito de qualidade.
...
- `compartilhar_mesma_campanha_entre_varias_lojas_iniciado`: início intencional do fluxo.
- `compartilhar_mesma_campanha_entre_varias_lojas_concluido`: após confirmação do resultado.
- `compartilhar_mesma_campanha_entre_varias_lojas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-025 — O supermercado deve poder compartilhar uma mesma campanha entre várias lojas.`
