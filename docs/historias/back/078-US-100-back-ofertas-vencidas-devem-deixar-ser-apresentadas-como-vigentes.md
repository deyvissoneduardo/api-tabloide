# US-100 — Ofertas vencidas devem deixar de ser apresentadas como vigentes
...
- **Dependências:** Back/US-056, Back/US-078, Back/US-068
- **Bloqueia:** Nenhuma
- **Risco:** Baixo
...
Estimativa relativa baseada no escopo descrito, nos cenários de validação e teste, no risco baixo e em 3 dependência(s) identificada(s).

---

## 1. Visão geral

A capacidade deve implementar ofertas vencidas devem deixar de ser apresentadas como vigentes usando as regras aprovadas de promoções e ofertas. O problema resolvido é permitir um comportamento consistente, autorizado, testável e rastreável para responsável pelo produto. O resultado esperado é a conclusão observável da capacidade sem depender do documento de origem.
...
**Quero** ofertas vencidas devem deixar de ser apresentadas como vigentes  
**Para** controlar o ciclo de vida do recurso sem perder histórico

---

## 3. Contexto de negócio

Esta história pertence a **Supermercado**, domínio **Promoções e ofertas**, seção “B6. Ofertas”. O requisito de origem é SUP-RF-060. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar ofertas vencidas devem deixar de ser apresentadas como vigentes no escopo autorizado.
- Aplicar as regras de promoções e ofertas documentadas nesta história.
...
- DONO e OPERADOR podem gerenciar promoções.
- OPERADOR não pode acessar cadastros ou áreas administrativas fora do fluxo de promoções.
...
O gatilho ocorre quando responsável pelo produto solicita ofertas vencidas devem deixar de ser apresentadas como vigentes.

---

## 8. Fluxo principal

1. O ator seleciona o recurso e solicita ofertas vencidas devem deixar de ser apresentadas como vigentes.
2. O sistema valida autenticação, permissão, estado atual e impactos.
3. Quando a ação for crítica, apresenta confirmação explícita e bloqueia envio duplicado.
4. Após confirmação, executa a transição de forma atômica.
5. Registra ator, instante, antes, depois e resultado.
6. Devolve o novo estado e aplica imediatamente os efeitos dependentes.
...
**Aplicação:**  
Nos fluxos de promoções e ofertas afetados por esta história.
...
- **RF-001:** O sistema deve ofertas vencidas devem deixar de ser apresentadas como vigentes.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "ofertas vencidas devem deixar de ser apresentadas como vigentes"
...
Rota: `/api/ofertas/{id}/acoes/{acao}`

Request: ação explícita e chave de idempotência quando crítica.

Sucesso: `200 OK`, com estado resultante.
...
- produto: obrigatório.
- preço normal e promocional: positivos, duas casas; promocional menor.
- início e fim: obrigatórios, horário de Brasília.
- lojas: uma ou mais.
- condições: opcionais, até 500 caracteres.
...
RASCUNHO -> publicação futura -> AGENDADA; AGENDADA -> início -> VIGENTE; VIGENTE -> fim -> EXPIRADA; VIGENTE -> desativação -> DESATIVADA; qualquer estado não final -> cancelamento -> CANCELADA.
...
- `ofertas_vencidas_devem_deixar_ser_apresentadas_como_vigentes_iniciado`: início intencional do fluxo.
- `ofertas_vencidas_devem_deixar_ser_apresentadas_como_vigentes_concluido`: após confirmação do resultado.
- `ofertas_vencidas_devem_deixar_ser_apresentadas_como_vigentes_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-060 — Ofertas vencidas devem deixar de ser apresentadas como vigentes.`
- Decisões aplicáveis: seção “Promoções e ofertas” de `docs/historias/decisoes-mvp.md`.
- Histórias relacionadas: US-056, US-078, US-068.
