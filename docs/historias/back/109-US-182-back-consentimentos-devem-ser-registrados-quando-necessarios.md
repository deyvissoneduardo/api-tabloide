# US-182 — Consentimentos devem ser registrados quando necessários
...
Esta capacidade foi analisada e retirada do comportamento ativo do MVP pelas decisões aprovadas. A história permanece para rastreabilidade do item original e deve comprovar que o sistema não oferece esse fluxo. O problema resolvido é permitir um comportamento consistente, autorizado, testável e rastreável para usuário autorizado do supermercado. O resultado esperado é ausência controlada da capacidade no produto.
...
**Quero** que o MVP não disponibilize consentimentos devem ser registrados quando necessários  
**Para** manter o escopo aprovado sem funcionalidades não contratadas

---

## 3. Contexto de negócio

Esta história pertence a **Supermercado**, domínio **Privacidade e consentimento**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RNF-029. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Garantir que menus, APIs e automações não exponham a capacidade retirada.
- Aplicar as regras de privacidade e consentimento documentadas nesta história.
...
O gatilho ocorre quando uma interface, API ou automação tenta disponibilizar a capacidade retirada do MVP.

---

## 8. Fluxo principal

1. O usuário acessa a área que poderia conter a capacidade.
2. A interface não apresenta menu, controle ou formulário para a capacidade retirada.
3. Se uma rota equivalente for chamada diretamente, o backend responde que a funcionalidade não está disponível no MVP.
4. Nenhuma configuração, consentimento ou alteração é persistida.
5. O sistema mantém as regras vigentes: mesmas funcionalidades para todos os planos e nenhuma coleta opcional de dados do consumidor.
...
**Aplicação:**  
Nos fluxos de privacidade e consentimento afetados por esta história.
...
- **RF-001:** O sistema deve impedir a disponibilização de consentimentos devem ser registrados quando necessários.
...
```gherkin
Cenário: Capacidade retirada permanece indisponível
  Dado que o ator está no escopo autorizado
  E o recurso atende às regras desta história
  Quando solicitar "consentimentos devem ser registrados quando necessários"
  Então o sistema deve recusar a capacidade como não disponível no MVP
...
Não existe endpoint para esta capacidade no MVP. Uma rota equivalente não deve persistir dados e deve seguir o padrão de recurso não disponível definido pela API.
...
- Nenhum dado próprio é criado ou alterado por esta história.
- Não existe configuração de funcionalidades por plano ou supermercado no MVP.
- Não existem registros de consentimento do consumidor porque não há coleta opcional nem cookies não essenciais.
- Testes devem comprovar a ausência de persistência e exposição do fluxo.

---

## 17. Estados e transições

Não aplicável: a capacidade não possui estado porque não está disponível no MVP.
...
- `consentimentos_devem_ser_registrados_quando_necessarios_iniciado`: início intencional do fluxo.
- `consentimentos_devem_ser_registrados_quando_necessarios_concluido`: após confirmação do resultado.
- `consentimentos_devem_ser_registrados_quando_necessarios_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RNF-029 — Consentimentos devem ser registrados quando necessários.`
- Decisões aplicáveis: seção “Privacidade e consentimento” de `docs/historias/decisoes-mvp.md`.
