...
Esta história pertence a **Supermercado**, domínio **Promoções e ofertas**, seção “B6. Ofertas”. O requisito de origem é SUP-RF-052. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar editar uma oferta no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita editar uma oferta.

---

## 8. Fluxo principal

1. O ator inicia editar uma oferta.
...
- **RF-001:** O sistema deve editar uma oferta.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "editar uma oferta"
...
- `editar_oferta_iniciado`: início intencional do fluxo.
- `editar_oferta_concluido`: após confirmação do resultado.
- `editar_oferta_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-052 — O supermercado deve poder editar uma oferta.`
