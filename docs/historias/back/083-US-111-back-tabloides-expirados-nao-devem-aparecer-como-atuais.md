# US-111 — Tabloides expirados não devem aparecer como atuais
...
**Quero** tabloides expirados não devem aparecer como atuais  
...
Esta história pertence a **Supermercado**, domínio **Tabloide existente**, seção “B19. Tabloide existente”. O requisito de origem é SUP-RF-164. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar tabloides expirados não devem aparecer como atuais no escopo autorizado.
- Aplicar as regras de tabloide existente documentadas nesta história.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita tabloides expirados não devem aparecer como atuais.

---

## 8. Fluxo principal

1. O ator seleciona o recurso e solicita tabloides expirados não devem aparecer como atuais.
...
**Aplicação:**  
Nos fluxos de tabloide existente afetados por esta história.
...
- **RF-001:** O sistema deve tabloides expirados não devem aparecer como atuais.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "tabloides expirados não devem aparecer como atuais"
...
Rota: `/api/tabloides/{id}/acoes/{acao}`
...
- título: obrigatório.
- PDF até 20 MB ou imagens JPG/PNG/WebP até 5 MB.
- início, fim e lojas: obrigatórios.
...
- `tabloides_expirados_nao_devem_aparecer_como_atuais_iniciado`: início intencional do fluxo.
- `tabloides_expirados_nao_devem_aparecer_como_atuais_concluido`: após confirmação do resultado.
- `tabloides_expirados_nao_devem_aparecer_como_atuais_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-164 — Tabloides expirados não devem aparecer como atuais.`
- Decisões aplicáveis: seção “Tabloide existente” de `docs/historias/decisoes-mvp.md`.
