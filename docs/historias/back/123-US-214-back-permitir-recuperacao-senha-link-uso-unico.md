# US-214 — Permitir recuperação de senha por link de uso único
...
**Quero** permitir recuperação de senha por link de uso único  
...
Esta história pertence a **Supermercado**, domínio **Identidade e acesso**, seção “Conta e acesso”. O requisito de origem é DEC-RF-007. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar permitir recuperação de senha por link de uso único no escopo autorizado.
...
2. O sistema aplica a qualidade “O sistema deve permitir recuperação de senha por link de uso único” antes de devolver o resultado.
...
- **RF-001:** O sistema deve permitir recuperação de senha por link de uso único.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "permitir recuperação de senha por link de uso único"
...
- `permitir_recuperacao_senha_link_uso_unico_iniciado`: início intencional do fluxo.
- `permitir_recuperacao_senha_link_uso_unico_concluido`: após confirmação do resultado.
- `permitir_recuperacao_senha_link_uso_unico_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-007 — O sistema deve permitir recuperação de senha por link de uso único.`
