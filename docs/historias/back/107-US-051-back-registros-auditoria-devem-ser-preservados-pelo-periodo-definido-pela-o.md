# US-051 — Registros de auditoria devem ser preservados pelo período definido pela operação
...
**Quero** registros de auditoria devem ser preservados pelo período definido pela operação  
...
Esta história pertence a **Administração da plataforma**, domínio **Auditoria**. O requisito de origem é ADM-RNF-013. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar registros de auditoria devem ser preservados pelo período definido pela operação no escopo autorizado.
...
2. O sistema aplica a qualidade “Registros de auditoria devem ser preservados pelo período definido pela operação” antes de devolver o resultado.
...
- **RF-001:** O sistema deve registros de auditoria devem ser preservados pelo período definido pela operação.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "registros de auditoria devem ser preservados pelo período definido pela operação"
...
- `registros_auditoria_devem_ser_preservados_pelo_periodo_definido_pela_o_iniciado`: início intencional do fluxo.
- `registros_auditoria_devem_ser_preservados_pelo_periodo_definido_pela_o_concluido`: após confirmação do resultado.
- `registros_auditoria_devem_ser_preservados_pelo_periodo_definido_pela_o_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RNF-013 — Registros de auditoria devem ser preservados pelo período definido pela operação.`
