# US-015 — Definir limites de utilização por plano
...
**Quero** definir limites de utilização por plano  
...
Esta história pertence a **Administração da plataforma**, domínio **Planos e assinaturas**. O requisito de origem é ADM-RF-037. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar definir limites de utilização por plano no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita definir limites de utilização por plano.

---

## 8. Fluxo principal

1. O ator inicia definir limites de utilização por plano.
...
- **RF-001:** O sistema deve definir limites de utilização por plano.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "definir limites de utilização por plano"
...
- `definir_limites_utilizacao_plano_iniciado`: início intencional do fluxo.
- `definir_limites_utilizacao_plano_concluido`: após confirmação do resultado.
- `definir_limites_utilizacao_plano_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-037 — O administrador deve poder definir limites de utilização por plano.`
