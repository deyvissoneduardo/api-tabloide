# US-041 — Restringir usuários autorizados devem acessar a administração geral da plataforma
...
**Quero** restringir usuários autorizados devem acessar a administração geral da plataforma  
...
Esta história pertence a **Administração da plataforma**, domínio **Identidade e acesso**. O requisito de origem é ADM-RNF-001. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar restringir usuários autorizados devem acessar a administração geral da plataforma no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita restringir usuários autorizados devem acessar a administração geral da plataforma.

---

## 8. Fluxo principal

1. O ator acessa a consulta de restringir usuários autorizados devem acessar a administração geral da plataforma.
...
- **RF-001:** O sistema deve restringir usuários autorizados devem acessar a administração geral da plataforma.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "restringir usuários autorizados devem acessar a administração geral da plataforma"
...
Rota: `/api/sessoes` ou `/api/sessoes/{id}`, conforme coleção ou detalhe.
...
- `restringir_usuarios_autorizados_devem_acessar_administracao_geral_plat_iniciado`: início intencional do fluxo.
- `restringir_usuarios_autorizados_devem_acessar_administracao_geral_plat_concluido`: após confirmação do resultado.
- `restringir_usuarios_autorizados_devem_acessar_administracao_geral_plat_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RNF-001 — Apenas usuários autorizados devem acessar a administração geral da plataforma.`
