# US-047 — Operações críticas devem reduzir o risco de execução acidental
...
**Quero** operações críticas devem reduzir o risco de execução acidental  
...
Esta história pertence a **Administração da plataforma**, domínio **Requisitos técnicos e UX recomendados**. O requisito de origem é ADM-RNF-009. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar operações críticas devem reduzir o risco de execução acidental no escopo autorizado.
...
2. O sistema aplica a qualidade “Operações críticas devem reduzir o risco de execução acidental” antes de devolver o resultado.
...
- **RF-001:** O sistema deve operações críticas devem reduzir o risco de execução acidental.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "operações críticas devem reduzir o risco de execução acidental"
...
A capacidade é transversal e deve ser validada nos endpoints afetados de `/api/qualidade`. Não cria endpoint exclusivo quando puder ser aplicada como requisito de qualidade.
...
- `operacoes_criticas_devem_reduzir_risco_execucao_acidental_iniciado`: início intencional do fluxo.
- `operacoes_criticas_devem_reduzir_risco_execucao_acidental_concluido`: após confirmação do resultado.
- `operacoes_criticas_devem_reduzir_risco_execucao_acidental_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RNF-009 — Operações críticas devem reduzir o risco de execução acidental.`
