# US-007 — Reativar o acesso de um supermercado
...
Estimativa relativa baseada no escopo descrito, nos cenários de validação e teste, no risco médio e em 1 dependência(s) identificada(s).

---

## 1. Visão geral

A capacidade deve implementar reativar o acesso de um supermercado usando as regras aprovadas de supermercados e estados. O problema resolvido é permitir um comportamento consistente, autorizado, testável e rastreável para Super Admin do SGTM. O resultado esperado é a conclusão observável da capacidade sem depender do documento de origem.
...
**Quero** reativar o acesso de um supermercado  
...
Esta história pertence a **Administração da plataforma**, domínio **Supermercados e estados**. O requisito de origem é ADM-RF-014. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar reativar o acesso de um supermercado no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita reativar o acesso de um supermercado.

---

## 8. Fluxo principal

1. O ator seleciona o recurso e solicita reativar o acesso de um supermercado.
...
- **RF-001:** O sistema deve reativar o acesso de um supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "reativar o acesso de um supermercado"
...
- `reativar_acesso_supermercado_iniciado`: início intencional do fluxo.
- `reativar_acesso_supermercado_concluido`: após confirmação do resultado.
- `reativar_acesso_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-014 — O administrador deve poder reativar o acesso de um supermercado.`
