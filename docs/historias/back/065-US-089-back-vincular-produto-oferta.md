# US-089 — Vincular um produto a uma oferta
...
**Quero** vincular um produto a uma oferta  
...
Esta história pertence a **Supermercado**, domínio **Promoções e ofertas**, seção “B6. Ofertas”. O requisito de origem é SUP-RF-046. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar vincular um produto a uma oferta no escopo autorizado.
...
2. O sistema aplica a qualidade “O supermercado deve poder vincular um produto a uma oferta” antes de devolver o resultado.
...
- **RF-001:** O sistema deve vincular um produto a uma oferta.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "vincular um produto a uma oferta"
...
A capacidade é transversal e deve ser validada nos endpoints afetados de `/api/ofertas`. Não cria endpoint exclusivo quando puder ser aplicada como requisito de qualidade.
...
- `vincular_produto_oferta_iniciado`: início intencional do fluxo.
- `vincular_produto_oferta_concluido`: após confirmação do resultado.
- `vincular_produto_oferta_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-046 — O supermercado deve poder vincular um produto a uma oferta.`
