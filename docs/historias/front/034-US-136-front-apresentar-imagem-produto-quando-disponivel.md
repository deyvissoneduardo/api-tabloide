# US-136 — Apresentar imagem do produto quando disponível
...
**Quero** apresentar imagem do produto quando disponível  
...
Esta história pertence a **Supermercado**, domínio **Página pública**, seção “B8. Página pública do supermercado”. O requisito de origem é SUP-RF-084. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar apresentar imagem do produto quando disponível no escopo autorizado.
...
- Consumidor acessa sem autenticação.
- DONO e OPERADOR não recebem privilégios adicionais na visualização pública.
...
O gatilho ocorre quando responsável pelo produto solicita apresentar imagem do produto quando disponível.

---

## 8. Fluxo principal

1. O ator acessa a consulta de apresentar imagem do produto quando disponível.
...
- **RF-001:** O sistema deve apresentar imagem do produto quando disponível.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "apresentar imagem do produto quando disponível"
...
A tela deve apresentar apresentar imagem do produto quando disponível com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `apresentar_imagem_produto_quando_disponivel_iniciado`: início intencional do fluxo.
- `apresentar_imagem_produto_quando_disponivel_concluido`: após confirmação do resultado.
- `apresentar_imagem_produto_quando_disponivel_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-084 — A página deve apresentar imagem do produto quando disponível.`
