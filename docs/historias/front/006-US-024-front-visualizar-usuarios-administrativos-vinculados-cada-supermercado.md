# US-024 — Visualizar os usuários administrativos vinculados a cada supermercado
...
**Quero** visualizar os usuários administrativos vinculados a cada supermercado  
...
Esta história pertence a **Administração da plataforma**, domínio **Supermercados e estados**. O requisito de origem é ADM-RF-012. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar os usuários administrativos vinculados a cada supermercado no escopo autorizado.
- Aplicar as regras de supermercados e estados documentadas nesta história.
...
O gatilho ocorre quando Super Admin do SGTM solicita visualizar os usuários administrativos vinculados a cada supermercado.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar os usuários administrativos vinculados a cada supermercado.
...
**Aplicação:**  
Nos fluxos de supermercados e estados afetados por esta história.
...
- **RF-001:** O sistema deve visualizar os usuários administrativos vinculados a cada supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar os usuários administrativos vinculados a cada supermercado"
...
A tela deve apresentar visualizar os usuários administrativos vinculados a cada supermercado com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- CNPJ: obrigatório, único, 14 caracteres alfanuméricos normalizados.
- razão social e nome fantasia: obrigatórios.
- e-mail e telefone comerciais: obrigatórios.
- CEP, logradouro, número, bairro, município e UF: obrigatórios.
- complemento, logomarca e observações internas: opcionais.
...
A interface consome a API `/api/supermercados` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
ATIVO -> bloqueio -> BLOQUEADO; ATIVO/BLOQUEADO -> desativação -> DESATIVADO; BLOQUEADO/DESATIVADO -> reativação -> ATIVO.
...
- `visualizar_usuarios_administrativos_vinculados_cada_supermercado_iniciado`: início intencional do fluxo.
- `visualizar_usuarios_administrativos_vinculados_cada_supermercado_concluido`: após confirmação do resultado.
- `visualizar_usuarios_administrativos_vinculados_cada_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-012 — O administrador deve poder visualizar os usuários administrativos vinculados a cada supermercado.`
- Decisões aplicáveis: seção “Supermercados e estados” de `docs/historias/decisoes-mvp.md`.
