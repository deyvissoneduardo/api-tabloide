# US-152 — Visualizar ranking de ofertas
...
**Quero** visualizar ranking de ofertas  
...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B11. Analytics”. O requisito de origem é SUP-RF-116. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar ranking de ofertas no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita visualizar ranking de ofertas.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar ranking de ofertas.
...
- **RF-001:** O sistema deve visualizar ranking de ofertas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar ranking de ofertas"
...
A tela deve apresentar visualizar ranking de ofertas com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `visualizar_ranking_ofertas_iniciado`: início intencional do fluxo.
- `visualizar_ranking_ofertas_concluido`: após confirmação do resultado.
- `visualizar_ranking_ofertas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-116 — O supermercado deve poder visualizar ranking de ofertas.`
