...
Esta capacidade foi retirada da interface do MVP: a conta única do Super Admin será recuperada diretamente no banco de dados. A história permanece para rastreabilidade de ADM-RF-004 e para impedir que uma tela ou API pública de recuperação seja criada indevidamente.
...
**Quero** que a recuperação da minha conta não seja exposta na interface  
**Para** manter o processo operacional aprovado para o MVP

---

## 3. Contexto de negócio

Esta história pertence a **Administração da plataforma**, domínio **Identidade e acesso**. O requisito de origem é ADM-RF-004. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Não apresentar link, formulário ou rota frontend de recuperação do Super Admin.
- Manter a recuperação como procedimento operacional direto no banco de dados.
...
O gatilho ocorre quando a tela de login do Super Admin é apresentada ou quando alguém tenta acessar uma rota de recuperação.

---

## 8. Fluxo principal

1. O sistema apresenta o login do Super Admin com e-mail e senha.
2. A interface não apresenta opção “esqueci minha senha” nem formulário equivalente.
3. Se uma rota de recuperação for acessada diretamente, a aplicação não disponibiliza o fluxo.
4. A recuperação é executada operacionalmente no banco por pessoa autorizada.
5. Após a alteração, todas as sessões existentes do Super Admin são invalidadas.
...
- **RF-001:** O frontend não deve disponibilizar recuperação de acesso do Super Admin.
...
```gherkin
Cenário: Não exibir recuperação do Super Admin
  Dado que o ator está no escopo autorizado
  E o recurso atende às regras desta história
  Quando a tela de login for carregada
  Então não deve existir controle de recuperação de senha
  E uma rota direta não deve disponibilizar esse fluxo
```
...
A tela deve apresentar recuperar seu acesso com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
Não aplicável. O MVP não possui API frontend de recuperação do Super Admin; a alteração ocorre diretamente no banco de dados.
...
- `recuperar_acesso_iniciado`: início intencional do fluxo.
- `recuperar_acesso_concluido`: após confirmação do resultado.
- `recuperar_acesso_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-004 — O administrador deve poder recuperar seu acesso.`
