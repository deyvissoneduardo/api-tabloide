# US-142 — Visualizar a quantidade de scans de QR Code
...
**Quero** visualizar a quantidade de scans de qr code  
...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B11. Analytics”. O requisito de origem é SUP-RF-104. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar a quantidade de scans de qr code no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita visualizar a quantidade de scans de qr code.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar a quantidade de scans de qr code.
...
- **RF-001:** O sistema deve visualizar a quantidade de scans de qr code.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar a quantidade de scans de qr code"
...
A tela deve apresentar visualizar a quantidade de scans de qr code com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `visualizar_quantidade_scans_qr_code_iniciado`: início intencional do fluxo.
- `visualizar_quantidade_scans_qr_code_concluido`: após confirmação do resultado.
- `visualizar_quantidade_scans_qr_code_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-104 — O supermercado deve poder visualizar a quantidade de scans de QR Code.`
