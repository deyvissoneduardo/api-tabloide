...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B11. Analytics”. O requisito de origem é SUP-RF-114. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar comparar campanhas no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita comparar campanhas.

---

## 8. Fluxo principal

1. O ator acessa a consulta de comparar campanhas.
...
- **RF-001:** O sistema deve comparar campanhas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "comparar campanhas"
...
A tela deve apresentar comparar campanhas com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `comparar_campanhas_iniciado`: início intencional do fluxo.
- `comparar_campanhas_concluido`: após confirmação do resultado.
- `comparar_campanhas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-114 — O supermercado deve poder comparar campanhas.`
