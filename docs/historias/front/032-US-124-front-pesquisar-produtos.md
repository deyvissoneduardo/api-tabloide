...
Esta história pertence a **Supermercado**, domínio **Produtos**, seção “B5. Produtos”. O requisito de origem é SUP-RF-043. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar pesquisar produtos no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita pesquisar produtos.

---

## 8. Fluxo principal

1. O ator acessa a consulta de pesquisar produtos.
...
- **RF-001:** O sistema deve pesquisar produtos.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "pesquisar produtos"
...
A tela deve apresentar pesquisar produtos com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `pesquisar_produtos_iniciado`: início intencional do fluxo.
- `pesquisar_produtos_concluido`: após confirmação do resultado.
- `pesquisar_produtos_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-043 — O supermercado deve poder pesquisar produtos.`
