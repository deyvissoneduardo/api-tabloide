# US-223 — Solicitar a exportação de relatórios
...
**Quero** solicitar a exportação de relatórios  
...
Esta história pertence a **Administração da plataforma**, domínio **Analytics, dashboard e relatórios**, seção “Relatórios”. O requisito de origem é DEC-RF-016. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar solicitar a exportação de relatórios no escopo autorizado.
...
2. O sistema aplica a qualidade “O administrador deve solicitar a exportação de relatórios” antes de devolver o resultado.
...
- **RF-001:** O sistema deve solicitar a exportação de relatórios.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "solicitar a exportação de relatórios"
...
A tela deve apresentar solicitar a exportação de relatórios com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `solicitar_exportacao_relatorios_iniciado`: início intencional do fluxo.
- `solicitar_exportacao_relatorios_concluido`: após confirmação do resultado.
- `solicitar_exportacao_relatorios_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-016 — O administrador deve solicitar a exportação de relatórios.`
