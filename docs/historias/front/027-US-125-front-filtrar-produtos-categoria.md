# US-125 — Filtrar produtos por categoria
...
Esta história pertence a **Supermercado**, domínio **Produtos**, seção “B5. Produtos”. O requisito de origem é SUP-RF-044. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar filtrar produtos por categoria no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita filtrar produtos por categoria.

---

## 8. Fluxo principal

1. O ator acessa a consulta de filtrar produtos por categoria.
...
- **RF-001:** O sistema deve filtrar produtos por categoria.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "filtrar produtos por categoria"
...
A tela deve apresentar filtrar produtos por categoria com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/produtos` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `filtrar_produtos_categoria_iniciado`: início intencional do fluxo.
- `filtrar_produtos_categoria_concluido`: após confirmação do resultado.
- `filtrar_produtos_categoria_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-044 — O supermercado deve poder filtrar produtos por categoria.`
