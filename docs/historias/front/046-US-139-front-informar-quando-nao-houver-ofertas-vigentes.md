# US-139 — Informar quando não houver ofertas vigentes
...
**Quero** informar quando não houver ofertas vigentes  
...
Esta história pertence a **Supermercado**, domínio **Página pública**, seção “B8. Página pública do supermercado”. O requisito de origem é SUP-RF-087. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar informar quando não houver ofertas vigentes no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita informar quando não houver ofertas vigentes.

---

## 8. Fluxo principal

1. O ator inicia informar quando não houver ofertas vigentes.
...
- **RF-001:** O sistema deve informar quando não houver ofertas vigentes.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "informar quando não houver ofertas vigentes"
...
A tela deve apresentar informar quando não houver ofertas vigentes com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `informar_quando_nao_houver_ofertas_vigentes_iniciado`: início intencional do fluxo.
- `informar_quando_nao_houver_ofertas_vigentes_concluido`: após confirmação do resultado.
- `informar_quando_nao_houver_ofertas_vigentes_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-087 — A página deve informar quando não houver ofertas vigentes.`
