# US-147 — Selecionar um período de análise
...
**Quero** selecionar um período de análise  
...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B11. Analytics”. O requisito de origem é SUP-RF-110. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar selecionar um período de análise no escopo autorizado.
...
2. O sistema aplica a qualidade “O supermercado deve poder selecionar um período de análise” antes de devolver o resultado.
...
- **RF-001:** O sistema deve selecionar um período de análise.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "selecionar um período de análise"
...
A tela deve apresentar selecionar um período de análise com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `selecionar_periodo_analise_iniciado`: início intencional do fluxo.
- `selecionar_periodo_analise_concluido`: após confirmação do resultado.
- `selecionar_periodo_analise_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-110 — O supermercado deve poder selecionar um período de análise.`
