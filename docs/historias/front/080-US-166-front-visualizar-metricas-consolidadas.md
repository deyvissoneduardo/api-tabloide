# US-166 — Visualizar métricas consolidadas
...
**Quero** visualizar métricas consolidadas  
...
Esta história pertence a **Supermercado**, domínio **Multi-loja**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RF-175. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar métricas consolidadas no escopo autorizado.
...
O gatilho ocorre quando usuário DONO de uma rede solicita visualizar métricas consolidadas.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar métricas consolidadas.
...
- **RF-001:** O sistema deve visualizar métricas consolidadas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar métricas consolidadas"
...
A tela deve apresentar visualizar métricas consolidadas com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `visualizar_metricas_consolidadas_iniciado`: início intencional do fluxo.
- `visualizar_metricas_consolidadas_concluido`: após confirmação do resultado.
- `visualizar_metricas_consolidadas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-175 — A rede deve poder visualizar métricas consolidadas.`
