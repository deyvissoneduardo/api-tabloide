# US-039 — Consultar informações necessárias para suporte ao supermercado
...
**Quero** consultar informações necessárias para suporte ao supermercado  
...
Esta história pertence a **Administração da plataforma**, domínio **Suporte e ocorrências**. O requisito de origem é ADM-RF-034. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar consultar informações necessárias para suporte ao supermercado no escopo autorizado.
- Aplicar as regras de suporte e ocorrências documentadas nesta história.
...
O gatilho ocorre quando Super Admin do SGTM solicita consultar informações necessárias para suporte ao supermercado.

---

## 8. Fluxo principal

1. O ator acessa a consulta de consultar informações necessárias para suporte ao supermercado.
...
**Aplicação:**  
Nos fluxos de suporte e ocorrências afetados por esta história.
...
- **RF-001:** O sistema deve consultar informações necessárias para suporte ao supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "consultar informações necessárias para suporte ao supermercado"
...
A tela deve apresentar consultar informações necessárias para suporte ao supermercado com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- título, descrição e severidade: obrigatórios.
- status, responsável, supermercado/loja e datas.
- histórico de comentários e transições.
...
A interface consome a API `/api/ocorrencias` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
ABERTA -> início da análise -> EM_ANÁLISE; EM_ANÁLISE -> resolução -> RESOLVIDA; RESOLVIDA -> encerramento -> ENCERRADA.
...
- `consultar_informacoes_necessarias_suporte_ao_supermercado_iniciado`: início intencional do fluxo.
- `consultar_informacoes_necessarias_suporte_ao_supermercado_concluido`: após confirmação do resultado.
- `consultar_informacoes_necessarias_suporte_ao_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-034 — O administrador deve poder consultar informações necessárias para suporte ao supermercado.`
- Decisões aplicáveis: seção “Suporte e ocorrências” de `docs/historias/decisoes-mvp.md`.
