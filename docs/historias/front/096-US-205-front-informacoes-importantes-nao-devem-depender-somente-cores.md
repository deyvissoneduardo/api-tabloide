# US-205 — Informações importantes não devem depender somente de cores
...
- **Dependências:** Back/US-056, Back/US-220, Back/US-088
- **Bloqueia:** Nenhuma
- **Risco:** Baixo
...
**Quero** informações importantes não devem depender somente de cores  
...
Esta história pertence a **Supermercado**, domínio **Campanhas**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RNF-033. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar informações importantes não devem depender somente de cores no escopo autorizado.
- Aplicar as regras de campanhas documentadas nesta história.
...
2. O sistema aplica a qualidade “Informações importantes não devem depender somente de cores” antes de devolver o resultado.
...
**Aplicação:**  
Nos fluxos de campanhas afetados por esta história.
...
- **RF-001:** O sistema deve informações importantes não devem depender somente de cores.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "informações importantes não devem depender somente de cores"
...
A tela deve apresentar informações importantes não devem depender somente de cores com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/campanhas` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
RASCUNHO -> publicação futura -> AGENDADA; AGENDADA -> início -> VIGENTE; VIGENTE -> fim -> EXPIRADA; estado não final -> cancelamento -> CANCELADA.
...
- `informacoes_importantes_nao_devem_depender_somente_cores_iniciado`: início intencional do fluxo.
- `informacoes_importantes_nao_devem_depender_somente_cores_concluido`: após confirmação do resultado.
- `informacoes_importantes_nao_devem_depender_somente_cores_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RNF-033 — Informações importantes não devem depender somente de cores.`
- Decisões aplicáveis: seção “Campanhas” de `docs/historias/decisoes-mvp.md`.
- Histórias relacionadas: US-056, US-220, US-088.
