# US-191 — O consumidor não deve precisar de treinamento
...
**Como** consumidor  
**Quero** o consumidor não deve precisar de treinamento  
...
Esta história pertence a **Supermercado**, domínio **Requisitos técnicos e UX recomendados**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RNF-002. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar o consumidor não deve precisar de treinamento no escopo autorizado.
...
2. O sistema aplica a qualidade “O consumidor não deve precisar de treinamento” antes de devolver o resultado.
...
- **RF-001:** O sistema deve o consumidor não deve precisar de treinamento.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "o consumidor não deve precisar de treinamento"
...
A tela deve apresentar o consumidor não deve precisar de treinamento com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `consumidor_nao_deve_precisar_treinamento_iniciado`: início intencional do fluxo.
- `consumidor_nao_deve_precisar_treinamento_concluido`: após confirmação do resultado.
- `consumidor_nao_deve_precisar_treinamento_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RNF-002 — O consumidor não deve precisar de treinamento.`
