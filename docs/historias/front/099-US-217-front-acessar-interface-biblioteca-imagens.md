# US-217 — Acessar a interface da biblioteca de imagens
...
**Quero** acessar a interface da biblioteca de imagens  
...
Esta história pertence a **Supermercado**, domínio **Armazenamento e imagens**, seção “Imagens”. O requisito de origem é DEC-RF-010. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar acessar a interface da biblioteca de imagens no escopo autorizado.
- Aplicar as regras de armazenamento e imagens documentadas nesta história.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita acessar a interface da biblioteca de imagens.

---

## 8. Fluxo principal

1. O ator acessa a consulta de acessar a interface da biblioteca de imagens.
...
**Aplicação:**  
Nos fluxos de armazenamento e imagens afetados por esta história.
...
- **RF-001:** O sistema deve acessar a interface da biblioteca de imagens.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "acessar a interface da biblioteca de imagens"
...
A tela deve apresentar acessar a interface da biblioteca de imagens com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- arquivo: JPG, PNG ou WebP, máximo 5 MB.
- nome de busca e data de upload.
- vínculos com produto, banner, logomarca ou tabloide.
- cota vigente do supermercado.
...
A interface consome a API `/api/imagens` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `acessar_interface_biblioteca_imagens_iniciado`: início intencional do fluxo.
- `acessar_interface_biblioteca_imagens_concluido`: após confirmação do resultado.
- `acessar_interface_biblioteca_imagens_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-010 — O supermercado deve acessar a interface da biblioteca de imagens.`
- Decisões aplicáveis: seção “Armazenamento e imagens” de `docs/historias/decisoes-mvp.md`.
