# US-040 — Consultar relatórios gerais de utilização
...
**Quero** consultar relatórios gerais de utilização  
...
Esta história pertence a **Administração da plataforma**, domínio **Analytics, dashboard e relatórios**. O requisito de origem é ADM-RF-040. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar consultar relatórios gerais de utilização no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita consultar relatórios gerais de utilização.

---

## 8. Fluxo principal

1. O ator acessa a consulta de consultar relatórios gerais de utilização.
...
- **RF-001:** O sistema deve consultar relatórios gerais de utilização.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "consultar relatórios gerais de utilização"
...
A tela deve apresentar consultar relatórios gerais de utilização com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `consultar_relatorios_gerais_utilizacao_iniciado`: início intencional do fluxo.
- `consultar_relatorios_gerais_utilizacao_concluido`: após confirmação do resultado.
- `consultar_relatorios_gerais_utilizacao_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-040 — O administrador deve poder consultar relatórios gerais de utilização.`
