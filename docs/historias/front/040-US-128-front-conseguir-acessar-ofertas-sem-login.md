# US-128 — Conseguir acessar as ofertas sem login
...
**Como** consumidor  
**Quero** conseguir acessar as ofertas sem login  
...
Esta história pertence a **Supermercado**, domínio **Página pública**, seção “B8. Página pública do supermercado”. O requisito de origem é SUP-RF-074. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar conseguir acessar as ofertas sem login no escopo autorizado.
...
O gatilho ocorre quando consumidor solicita conseguir acessar as ofertas sem login.

---

## 8. Fluxo principal

1. O ator acessa a consulta de conseguir acessar as ofertas sem login.
...
- **RF-001:** O sistema deve conseguir acessar as ofertas sem login.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "conseguir acessar as ofertas sem login"
...
A tela deve apresentar conseguir acessar as ofertas sem login com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `conseguir_acessar_ofertas_sem_login_iniciado`: início intencional do fluxo.
- `conseguir_acessar_ofertas_sem_login_concluido`: após confirmação do resultado.
- `conseguir_acessar_ofertas_sem_login_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-074 — O consumidor deve conseguir acessar as ofertas sem login.`
