# US-135 — Apresentar validade da promoção
...
**Quero** apresentar validade da promoção  
...
Esta história pertence a **Supermercado**, domínio **Página pública**, seção “B8. Página pública do supermercado”. O requisito de origem é SUP-RF-083. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar apresentar validade da promoção no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita apresentar validade da promoção.

---

## 8. Fluxo principal

1. O ator acessa a consulta de apresentar validade da promoção.
...
- **RF-001:** O sistema deve apresentar validade da promoção.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "apresentar validade da promoção"
...
A tela deve apresentar apresentar validade da promoção com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `apresentar_validade_promocao_iniciado`: início intencional do fluxo.
- `apresentar_validade_promocao_concluido`: após confirmação do resultado.
- `apresentar_validade_promocao_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-083 — A página deve apresentar validade da promoção.`
