# US-023 — Visualizar as lojas vinculadas a cada supermercado
...
**Quero** visualizar as lojas vinculadas a cada supermercado  
...
Esta história pertence a **Administração da plataforma**, domínio **Supermercados e estados**. O requisito de origem é ADM-RF-011. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar as lojas vinculadas a cada supermercado no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita visualizar as lojas vinculadas a cada supermercado.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar as lojas vinculadas a cada supermercado.
...
- **RF-001:** O sistema deve visualizar as lojas vinculadas a cada supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar as lojas vinculadas a cada supermercado"
...
A tela deve apresentar visualizar as lojas vinculadas a cada supermercado com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `visualizar_lojas_vinculadas_cada_supermercado_iniciado`: início intencional do fluxo.
- `visualizar_lojas_vinculadas_cada_supermercado_concluido`: após confirmação do resultado.
- `visualizar_lojas_vinculadas_cada_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-011 — O administrador deve poder visualizar as lojas vinculadas a cada supermercado.`
