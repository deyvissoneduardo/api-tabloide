...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B11. Analytics”. O requisito de origem é SUP-RF-115. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar comparar lojas no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita comparar lojas.

---

## 8. Fluxo principal

1. O ator acessa a consulta de comparar lojas.
...
- **RF-001:** O sistema deve comparar lojas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "comparar lojas"
...
A tela deve apresentar comparar lojas com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `comparar_lojas_iniciado`: início intencional do fluxo.
- `comparar_lojas_concluido`: após confirmação do resultado.
- `comparar_lojas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-115 — O supermercado deve poder comparar lojas.`
