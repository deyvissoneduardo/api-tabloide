# US-129 — Conseguir acessar as ofertas sem cadastro
...
Estimativa relativa baseada no escopo descrito, nos cenários de validação e teste, no risco baixo e em não possuir dependência funcional obrigatória identificada.

---

## 1. Visão geral

A capacidade deve implementar conseguir acessar as ofertas sem cadastro usando as regras aprovadas de página pública. O problema resolvido é permitir um comportamento consistente, autorizado, testável e rastreável para consumidor. O resultado esperado é a conclusão observável da capacidade sem depender do documento de origem.
...
**Como** consumidor  
**Quero** conseguir acessar as ofertas sem cadastro  
...
Esta história pertence a **Supermercado**, domínio **Página pública**, seção “B8. Página pública do supermercado”. O requisito de origem é SUP-RF-075. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar conseguir acessar as ofertas sem cadastro no escopo autorizado.
- Aplicar as regras de página pública documentadas nesta história.
...
O gatilho ocorre quando consumidor solicita conseguir acessar as ofertas sem cadastro.

---

## 8. Fluxo principal

1. O ator acessa a consulta de conseguir acessar as ofertas sem cadastro.
...
**Aplicação:**  
Nos fluxos de página pública afetados por esta história.
...
- **RF-001:** O sistema deve conseguir acessar as ofertas sem cadastro.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "conseguir acessar as ofertas sem cadastro"
...
A tela deve apresentar conseguir acessar as ofertas sem cadastro com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/publico/ofertas` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `conseguir_acessar_ofertas_sem_cadastro_iniciado`: início intencional do fluxo.
- `conseguir_acessar_ofertas_sem_cadastro_concluido`: após confirmação do resultado.
- `conseguir_acessar_ofertas_sem_cadastro_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-075 — O consumidor deve conseguir acessar as ofertas sem cadastro.`
- Decisões aplicáveis: seção “Página pública” de `docs/historias/decisoes-mvp.md`.
