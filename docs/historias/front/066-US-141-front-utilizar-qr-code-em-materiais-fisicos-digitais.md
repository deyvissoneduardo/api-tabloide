# US-141 — Utilizar o QR Code em materiais físicos e digitais
...
**Quero** utilizar o qr code em materiais físicos e digitais  
...
Esta história pertence a **Supermercado**, domínio **QR Codes**, seção “B9. QR Code”. O requisito de origem é SUP-RF-095. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar utilizar o qr code em materiais físicos e digitais no escopo autorizado.
...
2. O sistema aplica a qualidade “O supermercado deve poder utilizar o QR Code em materiais físicos e digitais” antes de devolver o resultado.
...
- **RF-001:** O sistema deve utilizar o qr code em materiais físicos e digitais.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "utilizar o qr code em materiais físicos e digitais"
...
A tela deve apresentar utilizar o qr code em materiais físicos e digitais com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/qrcodes` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `utilizar_qr_code_em_materiais_fisicos_digitais_iniciado`: início intencional do fluxo.
- `utilizar_qr_code_em_materiais_fisicos_digitais_concluido`: após confirmação do resultado.
- `utilizar_qr_code_em_materiais_fisicos_digitais_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-095 — O supermercado deve poder utilizar o QR Code em materiais físicos e digitais.`
