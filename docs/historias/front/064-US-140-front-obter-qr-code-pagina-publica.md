# US-140 — Obter um QR Code para sua página pública
...
- **Bloqueia:** Back/US-102, Back/US-103, Back/US-104, Back/US-105, Back/US-106, Back/US-107, Front/US-141, Back/US-170, Back/US-188
- **Risco:** Médio
...
**Quero** obter um qr code para sua página pública  
...
Esta história pertence a **Supermercado**, domínio **QR Codes**, seção “B9. QR Code”. O requisito de origem é SUP-RF-088. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar obter um qr code para sua página pública no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita obter um qr code para sua página pública.

---

## 8. Fluxo principal

1. O ator acessa a consulta de obter um qr code para sua página pública.
...
- **RF-001:** O sistema deve obter um qr code para sua página pública.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "obter um qr code para sua página pública"
...
A tela deve apresentar obter um qr code para sua página pública com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `obter_qr_code_pagina_publica_iniciado`: início intencional do fluxo.
- `obter_qr_code_pagina_publica_concluido`: após confirmação do resultado.
- `obter_qr_code_pagina_publica_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-088 — O supermercado deve poder obter um QR Code para sua página pública.`
