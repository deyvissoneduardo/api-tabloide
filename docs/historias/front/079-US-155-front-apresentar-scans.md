...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B12. Dashboard”. O requisito de origem é SUP-RF-119. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar apresentar scans no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita apresentar scans.

---

## 8. Fluxo principal

1. O ator acessa a consulta de apresentar scans.
...
- **RF-001:** O sistema deve apresentar scans.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "apresentar scans"
...
A tela deve apresentar apresentar scans com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `apresentar_scans_iniciado`: início intencional do fluxo.
- `apresentar_scans_concluido`: após confirmação do resultado.
- `apresentar_scans_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-119 — O painel deve apresentar scans.`
