...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B12. Dashboard”. O requisito de origem é SUP-RF-118. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar apresentar acessos no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita apresentar acessos.

---

## 8. Fluxo principal

1. O ator acessa a consulta de apresentar acessos.
...
- **RF-001:** O sistema deve apresentar acessos.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "apresentar acessos"
...
A tela deve apresentar apresentar acessos com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `apresentar_acessos_iniciado`: início intencional do fluxo.
- `apresentar_acessos_concluido`: após confirmação do resultado.
- `apresentar_acessos_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-118 — O painel deve apresentar acessos.`
