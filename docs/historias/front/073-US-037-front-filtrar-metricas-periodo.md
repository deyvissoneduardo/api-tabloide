# US-037 — Filtrar métricas por período
...
**Quero** filtrar métricas por período  
...
Esta história pertence a **Administração da plataforma**, domínio **Analytics, dashboard e relatórios**. O requisito de origem é ADM-RF-029. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar filtrar métricas por período no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita filtrar métricas por período.

---

## 8. Fluxo principal

1. O ator acessa a consulta de filtrar métricas por período.
...
- **RF-001:** O sistema deve filtrar métricas por período.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "filtrar métricas por período"
...
A tela deve apresentar filtrar métricas por período com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `filtrar_metricas_periodo_iniciado`: início intencional do fluxo.
- `filtrar_metricas_periodo_concluido`: após confirmação do resultado.
- `filtrar_metricas_periodo_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-029 — O administrador deve poder filtrar métricas por período.`
