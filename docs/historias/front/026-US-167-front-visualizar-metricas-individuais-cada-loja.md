# US-167 — Visualizar métricas individuais de cada loja
...
A capacidade deve implementar visualizar métricas individuais de cada loja usando as regras aprovadas de multi-loja. O problema resolvido é permitir um comportamento consistente, autorizado, testável e rastreável para usuário DONO de uma rede. O resultado esperado é a conclusão observável da capacidade sem depender do documento de origem.

---

## 2. História de usuário

**Como** usuário DONO de uma rede  
**Quero** visualizar métricas individuais de cada loja  
...
Esta história pertence a **Supermercado**, domínio **Multi-loja**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RF-176. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar métricas individuais de cada loja no escopo autorizado.
- Aplicar as regras de multi-loja documentadas nesta história.
...
O gatilho ocorre quando usuário DONO de uma rede solicita visualizar métricas individuais de cada loja.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar métricas individuais de cada loja.
...
**Aplicação:**  
Nos fluxos de multi-loja afetados por esta história.
...
- **RF-001:** O sistema deve visualizar métricas individuais de cada loja.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar métricas individuais de cada loja"
...
A tela deve apresentar visualizar métricas individuais de cada loja com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/lojas` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `visualizar_metricas_individuais_cada_loja_iniciado`: início intencional do fluxo.
- `visualizar_metricas_individuais_cada_loja_concluido`: após confirmação do resultado.
- `visualizar_metricas_individuais_cada_loja_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-176 — A rede deve poder visualizar métricas individuais de cada loja.`
- Decisões aplicáveis: seção “Multi-loja” de `docs/historias/decisoes-mvp.md`.
