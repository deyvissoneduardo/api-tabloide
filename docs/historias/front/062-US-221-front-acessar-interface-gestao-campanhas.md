# US-221 — Acessar a interface de gestão de campanhas
...
**Como** usuário DONO  
**Quero** acessar a interface de gestão de campanhas  
...
Esta história pertence a **Supermercado**, domínio **Identidade e acesso**, seção “Campanhas”. O requisito de origem é DEC-RF-014. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar acessar a interface de gestão de campanhas no escopo autorizado.
...
O gatilho ocorre quando usuário DONO solicita acessar a interface de gestão de campanhas.

---

## 8. Fluxo principal

1. O ator acessa a consulta de acessar a interface de gestão de campanhas.
...
- **RF-001:** O sistema deve acessar a interface de gestão de campanhas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "acessar a interface de gestão de campanhas"
...
A tela deve apresentar acessar a interface de gestão de campanhas com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `acessar_interface_gestao_campanhas_iniciado`: início intencional do fluxo.
- `acessar_interface_gestao_campanhas_concluido`: após confirmação do resultado.
- `acessar_interface_gestao_campanhas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-014 — O DONO deve acessar a interface de gestão de campanhas.`
