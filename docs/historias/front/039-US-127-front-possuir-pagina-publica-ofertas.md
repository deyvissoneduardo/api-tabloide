# US-127 — Possuir uma página pública de ofertas
...
**Quero** possuir uma página pública de ofertas  
...
Esta história pertence a **Supermercado**, domínio **Página pública**, seção “B8. Página pública do supermercado”. O requisito de origem é SUP-RF-073. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar possuir uma página pública de ofertas no escopo autorizado.
...
2. O sistema aplica a qualidade “Cada supermercado ou loja deve possuir uma página pública de ofertas” antes de devolver o resultado.
...
- **RF-001:** O sistema deve possuir uma página pública de ofertas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "possuir uma página pública de ofertas"
...
A tela deve apresentar possuir uma página pública de ofertas com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `possuir_pagina_publica_ofertas_iniciado`: início intencional do fluxo.
- `possuir_pagina_publica_ofertas_concluido`: após confirmação do resultado.
- `possuir_pagina_publica_ofertas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-073 — Cada supermercado ou loja deve possuir uma página pública de ofertas.`
