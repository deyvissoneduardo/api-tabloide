# US-224 — Acessar a interface de gestão de planos
...
**Quero** acessar a interface de gestão de planos  
...
Esta história pertence a **Administração da plataforma**, domínio **Planos e assinaturas**, seção “Planos”. O requisito de origem é DEC-RF-017. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar acessar a interface de gestão de planos no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita acessar a interface de gestão de planos.

---

## 8. Fluxo principal

1. O ator acessa a consulta de acessar a interface de gestão de planos.
...
- **RF-001:** O sistema deve acessar a interface de gestão de planos.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "acessar a interface de gestão de planos"
...
A tela deve apresentar acessar a interface de gestão de planos com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `acessar_interface_gestao_planos_iniciado`: início intencional do fluxo.
- `acessar_interface_gestao_planos_concluido`: após confirmação do resultado.
- `acessar_interface_gestao_planos_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-017 — O Super Admin deve acessar a interface de gestão de planos.`
