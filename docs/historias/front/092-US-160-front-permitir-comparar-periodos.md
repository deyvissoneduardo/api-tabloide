# US-160 — Permitir comparar períodos
...
**Quero** permitir comparar períodos  
...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B12. Dashboard”. O requisito de origem é SUP-RF-125. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar permitir comparar períodos no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita permitir comparar períodos.

---

## 8. Fluxo principal

1. O ator acessa a consulta de permitir comparar períodos.
...
- **RF-001:** O sistema deve permitir comparar períodos.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "permitir comparar períodos"
...
A tela deve apresentar permitir comparar períodos com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `permitir_comparar_periodos_iniciado`: início intencional do fluxo.
- `permitir_comparar_periodos_concluido`: após confirmação do resultado.
- `permitir_comparar_periodos_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-125 — O painel deve permitir comparar períodos.`
