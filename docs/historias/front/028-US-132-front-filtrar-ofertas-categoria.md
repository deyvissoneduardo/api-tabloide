# US-132 — Filtrar ofertas por categoria
...
Esta história pertence a **Supermercado**, domínio **Página pública**, seção “B8. Página pública do supermercado”. O requisito de origem é SUP-RF-079. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar filtrar ofertas por categoria no escopo autorizado.
...
O gatilho ocorre quando consumidor solicita filtrar ofertas por categoria.

---

## 8. Fluxo principal

1. O ator acessa a consulta de filtrar ofertas por categoria.
...
- **RF-001:** O sistema deve filtrar ofertas por categoria.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "filtrar ofertas por categoria"
...
A tela deve apresentar filtrar ofertas por categoria com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `filtrar_ofertas_categoria_iniciado`: início intencional do fluxo.
- `filtrar_ofertas_categoria_concluido`: após confirmação do resultado.
- `filtrar_ofertas_categoria_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-079 — O consumidor deve poder filtrar ofertas por categoria.`
