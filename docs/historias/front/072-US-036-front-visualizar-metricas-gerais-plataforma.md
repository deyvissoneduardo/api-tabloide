# US-036 — Visualizar métricas gerais da plataforma
...
**Quero** visualizar métricas gerais da plataforma  
...
Esta história pertence a **Administração da plataforma**, domínio **Analytics, dashboard e relatórios**. O requisito de origem é ADM-RF-028. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar métricas gerais da plataforma no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita visualizar métricas gerais da plataforma.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar métricas gerais da plataforma.
...
- **RF-001:** O sistema deve visualizar métricas gerais da plataforma.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar métricas gerais da plataforma"
...
A tela deve apresentar visualizar métricas gerais da plataforma com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `visualizar_metricas_gerais_plataforma_iniciado`: início intencional do fluxo.
- `visualizar_metricas_gerais_plataforma_concluido`: após confirmação do resultado.
- `visualizar_metricas_gerais_plataforma_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-028 — O administrador deve poder visualizar métricas gerais da plataforma.`
