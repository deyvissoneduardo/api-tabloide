# US-196 — A navegação entre categorias deve ser simples
...
**Quero** a navegação entre categorias deve ser simples  
...
Esta história pertence a **Supermercado**, domínio **Requisitos técnicos e UX recomendados**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RNF-007. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar a navegação entre categorias deve ser simples no escopo autorizado.
...
2. O sistema aplica a qualidade “A navegação entre categorias deve ser simples” antes de devolver o resultado.
...
- **RF-001:** O sistema deve a navegação entre categorias deve ser simples.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "a navegação entre categorias deve ser simples"
...
A tela deve apresentar a navegação entre categorias deve ser simples com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `navegacao_entre_categorias_deve_ser_simples_iniciado`: início intencional do fluxo.
- `navegacao_entre_categorias_deve_ser_simples_concluido`: após confirmação do resultado.
- `navegacao_entre_categorias_deve_ser_simples_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RNF-007 — A navegação entre categorias deve ser simples.`
