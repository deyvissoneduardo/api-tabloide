# US-226 — Acessar a interface de conteúdos gerais
...
**Quero** acessar a interface de conteúdos gerais  
...
Esta história pertence a **Administração da plataforma**, domínio **Conteúdo geral da plataforma**, seção “Conteúdo geral”. O requisito de origem é DEC-RF-019. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar acessar a interface de conteúdos gerais no escopo autorizado.
- Aplicar as regras de conteúdo geral da plataforma documentadas nesta história.
...
O gatilho ocorre quando Super Admin do SGTM solicita acessar a interface de conteúdos gerais.

---

## 8. Fluxo principal

1. O ator acessa a consulta de acessar a interface de conteúdos gerais.
...
**Aplicação:**  
Nos fluxos de conteúdo geral da plataforma afetados por esta história.
...
- **RF-001:** O sistema deve acessar a interface de conteúdos gerais.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "acessar a interface de conteúdos gerais"
...
A tela deve apresentar acessar a interface de conteúdos gerais com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/conteudos-gerais` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
RASCUNHO -> publicação -> PUBLICADO; PUBLICADO -> substituição/retirada -> ARQUIVADO.
...
- `acessar_interface_conteudos_gerais_iniciado`: início intencional do fluxo.
- `acessar_interface_conteudos_gerais_concluido`: após confirmação do resultado.
- `acessar_interface_conteudos_gerais_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-019 — O Super Admin deve acessar a interface de conteúdos gerais.`
- Decisões aplicáveis: seção “Conteúdo geral da plataforma” de `docs/historias/decisoes-mvp.md`.
