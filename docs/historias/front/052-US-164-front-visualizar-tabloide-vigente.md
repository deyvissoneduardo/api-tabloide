# US-164 — Visualizar o tabloide vigente
...
**Como** consumidor  
**Quero** visualizar o tabloide vigente  
...
Esta história pertence a **Supermercado**, domínio **Tabloide existente**, seção “B19. Tabloide existente”. O requisito de origem é SUP-RF-163. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar o tabloide vigente no escopo autorizado.
...
O gatilho ocorre quando consumidor solicita visualizar o tabloide vigente.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar o tabloide vigente.
...
- **RF-001:** O sistema deve visualizar o tabloide vigente.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar o tabloide vigente"
...
A tela deve apresentar visualizar o tabloide vigente com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/tabloides` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `visualizar_tabloide_vigente_iniciado`: início intencional do fluxo.
- `visualizar_tabloide_vigente_concluido`: após confirmação do resultado.
- `visualizar_tabloide_vigente_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-163 — O consumidor deve poder visualizar o tabloide vigente.`
