# US-192 — O consumidor não deve precisar criar conta para visualizar ofertas
...
**Como** consumidor  
**Quero** o consumidor não deve precisar criar conta para visualizar ofertas  
...
Esta história pertence a **Supermercado**, domínio **Requisitos técnicos e UX recomendados**, seção “B21. Multi-loja / rede”. O requisito de origem é SUP-RNF-003. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar o consumidor não deve precisar criar conta para visualizar ofertas no escopo autorizado.
...
O gatilho ocorre quando consumidor solicita o consumidor não deve precisar criar conta para visualizar ofertas.

---

## 8. Fluxo principal

1. O ator inicia o consumidor não deve precisar criar conta para visualizar ofertas.
...
- **RF-001:** O sistema deve o consumidor não deve precisar criar conta para visualizar ofertas.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "o consumidor não deve precisar criar conta para visualizar ofertas"
...
A tela deve apresentar o consumidor não deve precisar criar conta para visualizar ofertas com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `consumidor_nao_deve_precisar_criar_conta_visualizar_ofertas_iniciado`: início intencional do fluxo.
- `consumidor_nao_deve_precisar_criar_conta_visualizar_ofertas_concluido`: após confirmação do resultado.
- `consumidor_nao_deve_precisar_criar_conta_visualizar_ofertas_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RNF-003 — O consumidor não deve precisar criar conta para visualizar ofertas.`
