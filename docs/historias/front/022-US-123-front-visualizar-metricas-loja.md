# US-123 — Visualizar métricas por loja
...
**Quero** visualizar métricas por loja  
...
Esta história pertence a **Supermercado**, domínio **Lojas**, seção “B3. Gestão de lojas”. O requisito de origem é SUP-RF-026. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar métricas por loja no escopo autorizado.
...
O gatilho ocorre quando responsável pelo produto solicita visualizar métricas por loja.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar métricas por loja.
...
- **RF-001:** O sistema deve visualizar métricas por loja.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar métricas por loja"
...
A tela deve apresentar visualizar métricas por loja com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `visualizar_metricas_loja_iniciado`: início intencional do fluxo.
- `visualizar_metricas_loja_concluido`: após confirmação do resultado.
- `visualizar_metricas_loja_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-026 — O supermercado deve poder visualizar métricas por loja.`
