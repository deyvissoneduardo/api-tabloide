# US-038 — Consultar registros de alterações relevantes
...
**Quero** consultar registros de alterações relevantes  
...
Esta história pertence a **Administração da plataforma**, domínio **Auditoria**. O requisito de origem é ADM-RF-032. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar consultar registros de alterações relevantes no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita consultar registros de alterações relevantes.

---

## 8. Fluxo principal

1. O ator acessa a consulta de consultar registros de alterações relevantes.
...
- **RF-001:** O sistema deve consultar registros de alterações relevantes.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "consultar registros de alterações relevantes"
...
A tela deve apresentar consultar registros de alterações relevantes com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/auditoria` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `consultar_registros_alteracoes_relevantes_iniciado`: início intencional do fluxo.
- `consultar_registros_alteracoes_relevantes_concluido`: após confirmação do resultado.
- `consultar_registros_alteracoes_relevantes_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-032 — O administrador deve poder consultar registros de alterações relevantes.`
