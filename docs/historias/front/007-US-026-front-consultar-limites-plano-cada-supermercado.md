# US-026 — Consultar os limites do plano de cada supermercado
...
**Quero** consultar os limites do plano de cada supermercado  
...
Esta história pertence a **Administração da plataforma**, domínio **Planos e assinaturas**. O requisito de origem é ADM-RF-018. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar consultar os limites do plano de cada supermercado no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita consultar os limites do plano de cada supermercado.

---

## 8. Fluxo principal

1. O ator acessa a consulta de consultar os limites do plano de cada supermercado.
...
- **RF-001:** O sistema deve consultar os limites do plano de cada supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "consultar os limites do plano de cada supermercado"
...
A tela deve apresentar consultar os limites do plano de cada supermercado com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `consultar_limites_plano_cada_supermercado_iniciado`: início intencional do fluxo.
- `consultar_limites_plano_cada_supermercado_concluido`: após confirmação do resultado.
- `consultar_limites_plano_cada_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-018 — O administrador deve poder consultar os limites do plano de cada supermercado.`
