# US-035 — Visualizar a quantidade total de leituras de QR Code
...
**Quero** visualizar a quantidade total de leituras de qr code  
...
Esta história pertence a **Administração da plataforma**, domínio **Analytics, dashboard e relatórios**. O requisito de origem é ADM-RF-027. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar visualizar a quantidade total de leituras de qr code no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita visualizar a quantidade total de leituras de qr code.

---

## 8. Fluxo principal

1. O ator acessa a consulta de visualizar a quantidade total de leituras de qr code.
...
- **RF-001:** O sistema deve visualizar a quantidade total de leituras de qr code.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "visualizar a quantidade total de leituras de qr code"
...
A tela deve apresentar visualizar a quantidade total de leituras de qr code com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `visualizar_quantidade_total_leituras_qr_code_iniciado`: início intencional do fluxo.
- `visualizar_quantidade_total_leituras_qr_code_concluido`: após confirmação do resultado.
- `visualizar_quantidade_total_leituras_qr_code_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-027 — O administrador deve poder visualizar a quantidade total de leituras de QR Code.`
