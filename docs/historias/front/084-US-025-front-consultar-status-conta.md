# US-025 — Consultar o status de uma conta
...
**Quero** consultar o status de uma conta  
...
Esta história pertence a **Administração da plataforma**, domínio **Supermercados e estados**. O requisito de origem é ADM-RF-015. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar consultar o status de uma conta no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita consultar o status de uma conta.

---

## 8. Fluxo principal

1. O ator acessa a consulta de consultar o status de uma conta.
...
- **RF-001:** O sistema deve consultar o status de uma conta.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "consultar o status de uma conta"
...
A tela deve apresentar consultar o status de uma conta com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `consultar_status_conta_iniciado`: início intencional do fluxo.
- `consultar_status_conta_concluido`: após confirmação do resultado.
- `consultar_status_conta_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-015 — O administrador deve poder consultar o status de uma conta.`
