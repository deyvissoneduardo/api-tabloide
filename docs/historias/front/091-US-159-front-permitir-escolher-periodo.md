# US-159 — Permitir escolher período
...
**Quero** permitir escolher período  
...
Esta história pertence a **Supermercado**, domínio **Analytics, dashboard e relatórios**, seção “B12. Dashboard”. O requisito de origem é SUP-RF-124. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar permitir escolher período no escopo autorizado.
...
2. O sistema aplica a qualidade “O painel deve permitir escolher período” antes de devolver o resultado.
...
- **RF-001:** O sistema deve permitir escolher período.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "permitir escolher período"
...
A tela deve apresentar permitir escolher período com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `permitir_escolher_periodo_iniciado`: início intencional do fluxo.
- `permitir_escolher_periodo_concluido`: após confirmação do resultado.
- `permitir_escolher_periodo_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-124 — O painel deve permitir escolher período.`
