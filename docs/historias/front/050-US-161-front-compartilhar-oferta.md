# US-161 — Compartilhar uma oferta
...
Esta história pertence a **Supermercado**, domínio **Compartilhamento**, seção “B15. Compartilhamento”. O requisito de origem é SUP-RF-138. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar compartilhar uma oferta no escopo autorizado.
...
2. O sistema aplica a qualidade “O consumidor deve poder compartilhar uma oferta” antes de devolver o resultado.
...
- **RF-001:** O sistema deve compartilhar uma oferta.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "compartilhar uma oferta"
...
A tela deve apresentar compartilhar uma oferta com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `compartilhar_oferta_iniciado`: início intencional do fluxo.
- `compartilhar_oferta_concluido`: após confirmação do resultado.
- `compartilhar_oferta_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-138 — O consumidor deve poder compartilhar uma oferta.`
