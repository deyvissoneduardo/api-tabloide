...
Esta história pertence a **Supermercado**, domínio **Identidade e acesso**, seção “B1. Conta e acesso do supermercado”. O requisito de origem é SUP-RF-003. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar encerrar sua sessão no escopo autorizado.
...
O gatilho ocorre quando usuário autorizado do supermercado solicita encerrar sua sessão.

---

## 8. Fluxo principal

1. O ator seleciona o recurso e solicita encerrar sua sessão.
...
- **RF-001:** O sistema deve encerrar sua sessão.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "encerrar sua sessão"
...
A tela deve apresentar encerrar sua sessão com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `encerrar_sessao_iniciado`: início intencional do fluxo.
- `encerrar_sessao_concluido`: após confirmação do resultado.
- `encerrar_sessao_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-003 — O usuário deve poder encerrar sua sessão.`
