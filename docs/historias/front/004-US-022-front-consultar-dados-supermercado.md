# US-022 — Consultar os dados de um supermercado
...
**Quero** consultar os dados de um supermercado  
...
Esta história pertence a **Administração da plataforma**, domínio **Supermercados e estados**. O requisito de origem é ADM-RF-006. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar consultar os dados de um supermercado no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita consultar os dados de um supermercado.

---

## 8. Fluxo principal

1. O ator acessa a consulta de consultar os dados de um supermercado.
...
- **RF-001:** O sistema deve consultar os dados de um supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "consultar os dados de um supermercado"
...
A tela deve apresentar consultar os dados de um supermercado com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `consultar_dados_supermercado_iniciado`: início intencional do fluxo.
- `consultar_dados_supermercado_concluido`: após confirmação do resultado.
- `consultar_dados_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-006 — O administrador deve poder consultar os dados de um supermercado.`
