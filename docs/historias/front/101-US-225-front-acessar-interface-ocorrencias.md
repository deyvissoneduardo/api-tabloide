# US-225 — Acessar a interface de ocorrências
...
**Quero** acessar a interface de ocorrências  
...
Esta história pertence a **Administração da plataforma**, domínio **Suporte e ocorrências**, seção “Suporte”. O requisito de origem é DEC-RF-018. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar acessar a interface de ocorrências no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita acessar a interface de ocorrências.

---

## 8. Fluxo principal

1. O ator acessa a consulta de acessar a interface de ocorrências.
...
- **RF-001:** O sistema deve acessar a interface de ocorrências.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "acessar a interface de ocorrências"
...
A tela deve apresentar acessar a interface de ocorrências com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `acessar_interface_ocorrencias_iniciado`: início intencional do fluxo.
- `acessar_interface_ocorrencias_concluido`: após confirmação do resultado.
- `acessar_interface_ocorrencias_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-018 — O Super Admin deve acessar a interface de ocorrências.`
