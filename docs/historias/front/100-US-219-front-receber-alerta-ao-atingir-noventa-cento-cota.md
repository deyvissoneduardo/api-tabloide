# US-219 — Receber alerta ao atingir noventa por cento da cota
...
**Como** usuário DONO  
**Quero** receber alerta ao atingir noventa por cento da cota  
...
Esta história pertence a **Supermercado**, domínio **Identidade e acesso**, seção “Armazenamento”. O requisito de origem é DEC-RF-012. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar receber alerta ao atingir noventa por cento da cota no escopo autorizado.
...
2. O sistema aplica a qualidade “O DONO deve receber alerta ao atingir noventa por cento da cota” antes de devolver o resultado.
...
- **RF-001:** O sistema deve receber alerta ao atingir noventa por cento da cota.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "receber alerta ao atingir noventa por cento da cota"
...
A tela deve apresentar receber alerta ao atingir noventa por cento da cota com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
- `receber_alerta_ao_atingir_noventa_cento_cota_iniciado`: início intencional do fluxo.
- `receber_alerta_ao_atingir_noventa_cento_cota_concluido`: após confirmação do resultado.
- `receber_alerta_ao_atingir_noventa_cento_cota_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `DEC-RF-012 — O DONO deve receber alerta ao atingir noventa por cento da cota.`
