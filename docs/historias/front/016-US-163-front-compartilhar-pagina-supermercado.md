# US-163 — Compartilhar a página do supermercado
...
**Como** consumidor  
**Quero** compartilhar a página do supermercado  
...
Esta história pertence a **Supermercado**, domínio **Compartilhamento**, seção “B15. Compartilhamento”. O requisito de origem é SUP-RF-140. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar compartilhar a página do supermercado no escopo autorizado.
- Aplicar as regras de compartilhamento documentadas nesta história.
...
2. O sistema aplica a qualidade “O consumidor deve poder compartilhar a página do supermercado” antes de devolver o resultado.
...
**Aplicação:**  
Nos fluxos de compartilhamento afetados por esta história.
...
- **RF-001:** O sistema deve compartilhar a página do supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "compartilhar a página do supermercado"
...
A tela deve apresentar compartilhar a página do supermercado com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/compartilhamentos` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `compartilhar_pagina_supermercado_iniciado`: início intencional do fluxo.
- `compartilhar_pagina_supermercado_concluido`: após confirmação do resultado.
- `compartilhar_pagina_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `SUP-RF-140 — O consumidor deve poder compartilhar a página do supermercado.`
- Decisões aplicáveis: seção “Compartilhamento” de `docs/historias/decisoes-mvp.md`.
