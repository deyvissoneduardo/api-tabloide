# US-027 — Consultar o estado da assinatura de cada supermercado
...
**Quero** consultar o estado da assinatura de cada supermercado  
...
Esta história pertence a **Administração da plataforma**, domínio **Planos e assinaturas**. O requisito de origem é ADM-RF-019. As decisões de 24/08/2026 têm precedência sobre interpretações anteriores e foram incorporadas abaixo.
...
- Executar consultar o estado da assinatura de cada supermercado no escopo autorizado.
...
O gatilho ocorre quando Super Admin do SGTM solicita consultar o estado da assinatura de cada supermercado.

---

## 8. Fluxo principal

1. O ator acessa a consulta de consultar o estado da assinatura de cada supermercado.
...
- **RF-001:** O sistema deve consultar o estado da assinatura de cada supermercado.
...
Cenário: Executar com dados e estado válidos
  ...
  Quando solicitar "consultar o estado da assinatura de cada supermercado"
...
A tela deve apresentar consultar o estado da assinatura de cada supermercado com título, contexto atual, controles autorizados e linguagem de varejo. Listas usam pesquisa, filtros e paginação; formulários preservam valores após erro. A ação principal fica desabilitada durante envio.
...
A interface consome a API `/api/planos` usando o método correspondente à ação. Deve tratar 200/201, 400/422, 401, 403, 404, 409 e 500 quando aplicáveis, sem inferir sucesso antes da resposta.
...
- `consultar_estado_assinatura_cada_supermercado_iniciado`: início intencional do fluxo.
- `consultar_estado_assinatura_cada_supermercado_concluido`: após confirmação do resultado.
- `consultar_estado_assinatura_cada_supermercado_erro`: término sem conclusão, com categoria não sensível.
...
- Item de origem: `ADM-RF-019 — O administrador deve poder consultar o estado da assinatura de cada supermercado.`
