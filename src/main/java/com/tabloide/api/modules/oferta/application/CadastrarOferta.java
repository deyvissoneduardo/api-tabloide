...
public class CadastrarOferta {
    ...
    @Transactional
    public Oferta executar(Long supermercadoId, DadosOferta dados, Long atorId, Perfil perfilAtor, Long supermercadoIdAtor) {
        ...
        Oferta oferta = Oferta.cadastrar(
                supermercadoId, dados.produtoId(), dados.lojaIds(), dados.precoNormal(), dados.precoPromocional(),
                dados.inicio(), dados.fim(), dados.condicoes(), dados.confirmarPublicacao(), agora
        );
        Oferta salva = ofertaRepository.salvar(oferta);

        auditoriaRepository.registrar(new RegistroAuditoria(
                null, atorId, perfilAtor, supermercadoId, "OFERTA_CADASTRADA", "Oferta", salva.id(),
